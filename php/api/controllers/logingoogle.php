<?php
    ...
    class LoginGoogle {
        ...
        public static $iv = 'Sd5LzPt2fxW+rQfF'; // FALTA LA CADENA DE CARACTERES QUE ESTÁ EN EL CÓDIGO COMPLETO
        ...
        if ($config['test'] && in_array($token, self::$secretaria)) {
            ...
            try {
                ...
                if (!$payload) {
                    ...
                    die(json_encode(['error' => 'Invalid ID token']));
                }
            } catch (Exception $e) {
                header('HTTP/1.1 401 Unauthorized');
                die(json_encode(['error' => 'Token verification failed', 'details' => $e->getMessage()]));
            }
                }
